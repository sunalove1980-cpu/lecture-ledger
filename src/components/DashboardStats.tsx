  ...
  return (
    <section className="grid grid-cols-1 gap-3 md:grid-cols-12">
      <article className="relative min-h-[250px] overflow-hidden bg-[#171916] p-6 text-white md:col-span-7 sm:p-8">
        <div className="absolute -right-16 -top-24 h-56 w-56 rounded-full border-[44px] border-[#d9ff57]/10" />
        <div className="relative flex h-full flex-col justify-between gap-12">
          <div className="flex items-start justify-between"><div><p className="text-[10px] font-bold tracking-[0.18em] text-[#a8aaa2]">MONTHLY REVENUE</p><p className="mt-2 text-sm font-semibold text-[#d4d5cf]">이번 달 강의 수익</p></div><ArrowUpRight className="h-5 w-5 text-[#d9ff57]" /></div>
          <div><p className="break-all text-[clamp(2.15rem,6vw,4.2rem)] font-black leading-none tracking-[-0.075em]">{won(totalEarnings)}</p>
            <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-white/15 pt-4 text-xs"><span className="text-[#92948d]">지난달 {won(prevMonthTotal)}</span>{prevMonthTotal > 0 && <span className={diffPercent >= 0 ? 'font-bold text-[#d9ff57]' : 'font-bold text-[#ff8d76]'}>{diffPercent >= 0 ? '+' : ''}{diffPercent}%</span>}</div>
          </div>
        </div>
      </article>
      ...
      <article className="flex items-center justify-between bg-[#d9ff57] p-5 md:col-span-4"><div><p className="text-[10px] font-black tracking-[0.14em] text-[#4c571e]">NEXT ACTION</p><p className="mt-2 text-base font-black tracking-[-0.04em] text-[#171916]">미정산 {pendingCount}건 확인하기</p></div><ArrowUpRight className="h-6 w-6 text-[#171916]" /></article>
    </section>
  );
};
